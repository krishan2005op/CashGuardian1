...
=== VARIANCE DRIVERS (MoM Change) ===
Revenue Drivers: ${JSON.stringify(snapshot.variances?.income.slice(0, 3) || [])}
...
- **Overdue Details**: ${JSON.stringify(snapshot.overdueList || [])}
- **Contact Directory**: ${JSON.stringify(snapshot.contacts || {})}
- Provide source transparency (Internal vs External).
- Use local context for identifying region or product contributors if they exist in transaction descriptions.
`;
}
...
function getSnapshot(customDataset = null) {
  ...
  if (customDataset && customDataset.length > 0) {
    ...
    snapshot = {
      ...
      breakdown,
      variances,
      overdueList: cleanedData.filter(item => item.status === 'overdue').map(i => ({
        client: i.client,
        amount: i.amount,
